...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.map { case (_, qa) =>
      val q = qa.head._1
      val highScore = answerHighScore(qa.map(_._2).toArray)
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.flatMap { case (q, highScore) =>
      firstLangInTag(q.tags, langs) match {
        case None => None
        case Some(i) => Some(i * langSpread, highScore)
      }
    }
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    val clusterAvg = vectors.map(v => (findClosest(v, means), v))
      .groupByKey()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      val langScoresSize      = vs.groupBy(_._1).mapValues(_.size)
      val mostCommon          = langScoresSize.maxBy(_._2)
      val langLabel: String   = langs(mostCommon._1 / langSpread) // most common language in the cluster
      val clusterSize: Int    = vs.size
      val langPercent: Double = 100 * (mostCommon._2 / clusterSize).toDouble // percent of the questions in the most common language
