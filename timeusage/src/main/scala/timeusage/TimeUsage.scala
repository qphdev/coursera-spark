...
/*    val summaryTyped = timeUsageSummaryTyped(summaryDf)
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def relatedTo(col: String, cols: Seq[String]): Boolean = cols.exists(col.startsWith)

    val primNeedsCols = columnNames.filter( c => relatedTo(c, primaryNeeds) )
    val workCols = columnNames.filter( c => relatedTo(c, work) )
    val leisureCols = columnNames.diff( primNeedsCols ::: workCols ).filter( c => relatedTo(c, leisure) )

    (primNeedsCols.map(new Column(_)), workCols.map(new Column(_)), leisureCols.map(new Column(_)))
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when($"telfs" >= 1 and $"telfs" < 3, "working")
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"""
       |select working, sex, age, round(avg(primaryNeeds), 1) as primaryNeeds, round(avg(work), 1) as work,
       |round(avg(other), 1) as other
       |from $viewName
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round(d: Double): Double = math.round(d * 10).toDouble / 10

    summed.groupByKey(tu => (tu.working, tu.sex, tu.age))
      .agg(typed.avg[TimeUsageRow](r => r.primaryNeeds),
           typed.avg[TimeUsageRow](r => r.work),
           typed.avg[TimeUsageRow](r => r.other))
      .map(r => TimeUsageRow(r._1._1, r._1._2, r._1._3, round(r._2), round(r._3), round(r._4)))
      .orderBy($"working", $"sex", $"age")
  }
}
